...
export const CartItem = ({title, quantity, price, removeItem, editable}) => (
    <div className="cart-item">
        <span className="normalizedWidth">{title}</span>
        <span>{quantity}</span>
        <div>
            <span>{`$${price}`}</span>
            {editable && (<button onClick={removeItem} className="quantity-button">
                <a>
    ...
    return (
        <div>
            ...
            <div className="cart-container">
                ...
                {cartContent.map(item=>(<CartItem key={item.id} title={item.title} price={item.price} quantity={item.quantity} removeItem={()=>removeItem(item.id)} editable />))}
                ...
                <Link className="cart-action-button" to="/checkout">Siguiente: Datos de cliente >></Link>
            </div>)}
        </div>
    )
}
